...
export class EquipmentService {
  async createEquipment(actorRole: string, payload: any) {
    if (actorRole !== 'ADMIN') {
    ...
    return withTransaction(async (client) => {
      ...
      if (payload.default_technician_id) {
        const tech = await usersRepo.findById(
          client,
  ...
  async listEquipment(updatedAfter?: string) {
    const client = await pool.connect();
    try {
  ...
  async assertEquipmentAvailable(equipmentId: string) {
    const client = await pool.connect();
    try {
      const eq = await repo.findById(client, equipmentId);
      if (!eq) {

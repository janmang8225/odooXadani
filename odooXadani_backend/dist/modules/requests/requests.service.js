...
export class RequestsService {
    async createRequest(auth, payload) {
        if (payload.type === 'PREVENTIVE' &&
            auth.role !== 'MANAGER') {
            throw new ForbiddenError();
        }
        await equipmentService.assertEquipmentAvailable(payload.equipment_id);
        return withTransaction(async (client) => {
            const equipment = await equipmentService.getEquipmentById(payload.equipment_id);
    ...
    async assignRequest(auth, requestId, technicianId) {
        return withTransaction(async (client) => {
            ...
            await teamsService.assertTechnicianInTeam(technicianId, req.team_id);
    ...
    async repairRequest(auth, requestId, durationHours) {
        return withTransaction(async (client) => {
            const req = await repo.findById(client, requestId);
            if (!req) {
                throw new AppError(404, 'REQUEST_NOT_FOUND', 'Request not found');
    ...
    async scrapRequest(auth, requestId) {
        ...
        return withTransaction(async (client) => {
            ...
            if (!Array.isArray(allowedTransitions) || !allowedTransitions.includes('SCRAP')) {
                throw new ValidationError(`Cannot scrap request with status "${req.status}". Allowed transitions: ${allowedTransitions?.join(', ') || 'none'}`);
            }
            await repo.update(client, requestId, { status: 'SCRAP' });
            await client.query(`UPDATE equipment SET is_scrapped = true, updated_at = now()
         WHERE id = $1`, [req.equipment_id]);
        });
    }
}

...
export class TeamsService {
    async createTeam(actorRole, name) {
        if (actorRole !== 'ADMIN') {
    ...
    async addTechnician(actorRole, teamId, technicianId) {
        if (actorRole !== 'ADMIN') {
            throw new ForbiddenError();
        }
        return withTransaction(async (client) => {
    ...
    async removeTechnician(actorRole, teamId, technicianId) {
        if (actorRole !== 'ADMIN') {
            throw new ForbiddenError();
        }
        return withTransaction(async (client) => {
    ...
    async getTeamsForTechnician(technicianId) {
    ...
    async assertTechnicianInTeam(technicianId, teamId) {
        const client = await pool.connect();
        try {

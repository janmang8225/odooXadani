export class PollingRepository {
    async teamRequests(client, teamId, updatedAfter) {
    ...
    async technicianRequests(client, technicianId, updatedAfter) {
        const { rows } = await client.query(`
      SELECT *
      FROM maintenance_requests
    ...
    async equipmentOpenCount(client, equipmentId) {
        const { rows } = await client.query(`
    ...
    async preventiveCalendar(client, from, to, updatedAfter) {
        const { rows } = await client.query(`
      SELECT *
      FROM maintenance_requests
...
//# sourceMappingURL=polling.repository.js.map